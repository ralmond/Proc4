...
testthat::test_that("all.equal.Event works", {
  ...
  testthat::expect_false(isTRUE(all.equal(anEvent,anEvent1)))
})

testthat::test_that("Round trip test for as.json and parseObject.", {
  jEvent <- as.json(anEvent)
  pEvent <- parseObject(fromJSON(jEvent),"Event")
  testthat::expect_equal(anEvent,pEvent)
})
