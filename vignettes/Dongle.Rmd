...
```{js, fig.cap="Sample message in P4 format", eval=FALSE}
{
...
Example \@ref(exm:AuthorizedApps) shows the code that creates it. The
`app` and `active` fields should be counted as required and as
recommended. 

::: {.example #AuthorizedApps}
Mongo script to create AuthorizedApps collection.
```{js AuthorizedApps, eval=FALSE}
...
the field to false. Example \@ref(exm:shutdown) gives some example
syntaxes (to run from the mongo command shell).

:::{.example #shutdown}
Mongo scripts to signal processes to shutdown
```{js shutdown, eval=FALSE}
...
each. Example \@ref(exm:Players) gives the schema for this collection.

:::{.example #Players}
Players collection schema definition
```{js Players, eval=FALSE}
...
`Statistics` collection in the database. Example \@ref(exm:Statistics)
provides the schema for that collection.

::: {.example #Statistics}
Mongo script to create Statistics collection and indexes.
```{js Statistics, eval=FALSE}
...
for future expansion. Example \@ref(exm:Activities) provides its draft
...
::: {.example #Activites}
Mongo script to create Activities collection and indexes.
```{js, Activities, eval=FALSE}
...
Example \@ref(exm:Proc4ini). The applications are particularly important
...
::: {.example #Proc4ini}
Proc4.ini -- PHP-style parameter file.
```{php, Proc4ini, eval=FALSE}
...
by the bash script `LLtoP4` (Example \@ref(exm:LLtoP4)). The translation
...
::: {.example #LLtoP4}
Learning Locker to P4 JSON conversion script.
```{bash LLtoP4, eval=FALSE}
#!/bin/bash
...
process.) Example \@ref(exm:LLtoP4Loop) shows the listing.

::: {.example #LLtoP4Loop}
Shell process for transfering LL events to EI input queue.
```{bash LLtoP4Loop, eval=FALSE}
#!/bin/bash
IP=127.0.0.1
name=$1
starttime=$2
