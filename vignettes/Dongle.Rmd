...
Figure \[fig:P4Lamp\] shows the basic architecture. At its heart, it is
...
determined by the message. Listing \[lst:P4\] shows the data structure
in JSON format.
...
Listing \[lst:AuthorizedApps\] shows the code that creates it. The
...
the field to false. Listing \[lst:shutdown\] gives some example
...
each. Listing \[lst:Players\] gives the schema for this collection.
...
Note that this is a subset of the basic \code{Proc4} message
...
`Statistics` collection in the database. Listing \[lst:Statistics\]
...
for future expansion. Listing \[lst:Activities\] provides its draft
...
Listing \[lst:Proc4ini\]. The applications are particularly important
...
by the bash script `LLtoP4` (Listing \[lst:LLtoP4\]). The translation
...
process.) Listing \[lst:LLtoP4Loop\] shows the listing.
...
task. Also, for the Proc4 for R library, R [@R] will need to be
